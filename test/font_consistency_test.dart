...
/// Extracts codepoints from `assets/codicon.css` using the same regex the
/// generator (`tool/generate_fonts.dart`) uses to build the Dart icon
/// constants — keep the two in sync.
Set<int> _cssCodepoints() {
  final content = File('assets/codicon.css').readAsStringSync();
  final pattern = RegExp(
    r'\.codicon-([^:\s]+):{1,2}before\s*\{\s*content:\s*"\\([0-9a-fA-F]+)";?[^}]*\}',
  );
  return pattern
      .allMatches(content)
...
/// Parses the `cmap` table of `assets/codicon.ttf` and returns the set of
/// codepoints covered by its format 4 and format 12 subtables.
Set<int> _fontCodepoints() {
  final bytes = File('assets/codicon.ttf').readAsBytesSync();
  ...
  if (cmapOffset == null) {
    throw StateError('cmap table not found in assets/codicon.ttf');
  }
...
void main() {
  test('CSS, Dart and font codepoints are consistent', () {
    ...
    // nothing. Codicons is a small set (~540 distinct codepoints; alias
    // selectors share codepoints), so the floor is lower than larger sets.
    expect(cssCodepoints, isNotEmpty);
    expect(cssCodepoints.length, greaterThan(300));
    ...
    expect(
      ...
      isEmpty,
      reason:
          'Dart codepoints missing from assets/codicon.css: '
    ...
    expect(
      ...
      isEmpty,
      reason:
          'CSS codepoints missing from the generated Codicons class: '
    ...
    expect(
      ...
      isEmpty,
      reason:
          'CSS codepoints not covered by assets/codicon.ttf cmap: '
